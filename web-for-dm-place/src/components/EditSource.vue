<template>
  <div>
    ...
    <el-form
      :model="sourceForm"
      ref="sourceForm"
      :rules="sourceFormRule"
      v-loading="sourceLoad"
      class="new-style-main"
      enctype="multipart/form-data"
    >
      <el-form-item label="数据源ID" prop="ssId">
        <el-input v-model="ssId" value="ssId" :disabled="true"></el-input>
      </el-form-item>
      <el-form-item label="数据源描述" prop="ssDescribe">
        <el-input v-model="sourceForm.ssDescribe" prepend="sourceForm.ssDescribe"></el-input>
      </el-form-item>
      <el-form-item>
...
<script>
import { editNewSource } from '../api/realTimeTaskApi';
...
export default {
    ...
    return {
      ...
      sourceFormRule: {
        ssDescribe: [
          ...
          { max: 200, message: '数据源描述长度不能超过200字符', trigger: 'blur' },
        ],
      },
  ...
  methods: {
    ...
    submit() {
      ...
      formdata.ssId = this.ssId
      if (this.sourceForm.ssDescribe) {
        this.sourceLoad = true;
        editNewSource(formdata).then(() => {
          this.sourceLoad = false;
          this.$message.success({ message: '编辑数据源成功', showClose: true });
          this.goBack();
        }).catch(() => {
          this.sourceLoad = false;
        });
