...
(function($, window, document, undefined) {
	...
	jRange.prototype = {
		...
		onDrag: function(pointer, e) {
			...
			var position=Math.round((e.clientX - this.domNode.offset().left)/this.options.step)*this.options.step;
			this.domNode.trigger('change', [this, pointer, position]);
			var currentDate=$(".single-slider").val();
			pirorDate=currentDate;
			this.changeHistoryWp(currentDate);
		},
		...
		barClicked: function(e) {
			...
			//点击轴上的点，移动一个步长
			var x=Math.round(e.offsetX/this.options.step)*this.options.step;
			if (this.isSingle())
		...
		renderScale: function() {
			...
			for (var i = 0; i < s.length; i++) {
				...
				//str += '<span style="left: ' + (i * prc).toFixed(2) + '%">'+ '<ins></ins>'+ '</span>'
				str += '<span style="left: ' + i * prc + '%">' + '<ins></ins>'  + '</span>';
		...
		positionToValue: function(pos) {
			...
			return this.options.scale[index];
		},
